...
class AuthViewModel extends ChangeNotifier {
  ...
  Future<void> getOtp(BuildContext context) async {
    ...
    try {
      final url = Uri.parse(
        'https://control.msg91.com/api/v5/otp?template_id=$_templateId&mobile=91$_phoneNumber&authkey=$_msg91AuthKey',
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (body['type'] == 'success') {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('MSG91 Error: Using Mock OTP (123456)'),
            ),
          );
          startResendTimer();
          Navigator.pushNamed(context, '/otp');
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'MSG91 Error ${response.statusCode}: Using Mock OTP (123456)',
            ),
          ),
        );
        startResendTimer();
  ...
  Future<void> verifyOtp(BuildContext context) async {
    ...
    try {
      final url = Uri.parse(
        'https://control.msg91.com/api/v5/otp/verify?otp=$_otp&mobile=91$_phoneNumber&authkey=$_msg91AuthKey',
      );
      final response = await http.get(url);
      ...
      if (response.statusCode == 200) {
        final body = jsonDecode(response.body);
        if (body['type'] == 'success' ||
            body['message'] == 'OTP verified success fully') {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
        ...
        if (_otp == '123456') {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Invalid Mock OTP. Use 123456.')),
          );
        }
      }
    } catch (e) {
